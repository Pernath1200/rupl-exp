...
| **Pisanie** (type) | The **minimal pattern unit only** — a single form for conjugation; a phrase where the pattern *is* a phrase (*dobra kawa, w domu*); a frame for to-jest/questions. **HARD CAP (James 2026-08-06): a typed-whole answer is at most 3 words — AND must be a frozen chunk or pattern phrase.** A **composed sentence is a cloze at ANY length** (chunk + bolted-on phrase — *Jak było w pracy?* — or a conjugated question — *Co robiłaś wczoraj?*). Cloze = `mode:"cloze"`, `frame` with one `___`, `answer` = the missing word, blank on the **unit's teaching point**. Never ask the learner to type a full sentence in Pisanie. See `codex/REPAIR-QUEUE.md` for the conversion of pre-rule packs. |
